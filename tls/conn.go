...
func (c *ClientHelloPreset) apply(hello *clientHelloMsg) {
	...
	hello.ocspStapling = c.StatusRequest
	hello.scts = c.Sct
	hello.alpnProtocols = c.Alpn
	hello.supportedPoints = c.EcPointFormats
	hello.pskModes = c.PskKeyExchangeModes
	hello.supportedVersions = c.SupportedVersions
	hello.supportedCurves = c.SupportedGroups
	hello.ticketSupported = c.SessionTicket
...
func (c *Conn) readRecordOrCCS(expectChangeCipherSpec bool) error {
	...
	case recordTypeAlert:
		...
		c.recvAlerts = append(c.recvAlerts, alert(data[1]))
...
func (c *Conn) sendAlertLocked(err alert) error {
	c.sendAlerts = append(c.sendAlerts, err)
	switch err {
...
func (a Alert) GetDescription() string {
	return alertText[alert(a)]
}

func (a Alert) GetCode() uint8 {
	return uint8(a)
}
