...
func (c *Conn) makeClientHello() (*clientHelloMsg, []ecdheParameters, error) {
	...
	if config.ClientHelloPreset != nil {
		config.CipherSuites = config.ClientHelloPreset.Ciphers
		config.NextProtos = config.ClientHelloPreset.Alpn
		config.Renegotiation = config.ClientHelloPreset.Renegotiation
	...
	if len(hello.supportedVersions) > 0 && hello.supportedVersions[0] == VersionTLS13 {
		...
		if c.config.KeyShares > uint(len(curvePreferences)) {
		...
		for _, curveID := range curves {
			...
			for _, value := range GreaseValues {
				if curveID == CurveID(value) {
					keyshareIsGrease = true
					paramsSingle = &greaseParameters{
						curveID:    curveID,
						privateKey: []byte{},
						// JARMs pub key is [0x00,0x01,0x00] 0x0001 is length prefix
						publicKey: []byte{0x00},
					}
				}
			}
	...
	if config.ClientHelloPreset != nil {
		config.ClientHelloPreset.apply(hello)
	}
...
func (hs *clientHandshakeState) doFullHandshake() error {
	...
	if ok {
		...
		if err != nil {
			if hs.c.config.InsecureSkipVerify {
				c.errors = append(c.errors, err)
				c.sendAlerts = append(c.sendAlerts, alertUnexpectedMessage)
			} else {
				c.sendAlert(alertUnexpectedMessage)
				return err
			}
...
func (c *Conn) verifyServerCertificate(certificates [][]byte) error {
...

	{
		...
		if !c.config.InsecureSkipVerify && err != nil {
			...
			c.sendAlerts = append(c.sendAlerts, alertBadCertificate)
			c.errors = append(c.errors, errors.New("tls: invalid certificate chain: "+err.Error()))
		}
	}
