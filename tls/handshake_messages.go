...
func (m *clientHelloMsg) marshal() []byte {
	...
	b.AddUint24LengthPrefixed(func(b *cryptobyte.Builder) {
		...
		b.AddUint16LengthPrefixed(func(b *cryptobyte.Builder) {
			var extensionBytes = []cryptobyte.Builder{}
			...
			if m.recordSizeLimit > 0 {
				var b cryptobyte.Builder
				b.AddUint16(ExtensionRecordSizeLimit)
				b.AddUint16LengthPrefixed(func(b *cryptobyte.Builder) {
			...
			if m.greaseExtension > 0 {
				...
				for _, value := range GreaseValues {
					if m.greaseExtension == value {
						valid = true
						break
					}
				}
				if !valid {
					panic(fmt.Sprintf("Grease extension value (%d) not a grease value", m.greaseExtension))
				}

				var b cryptobyte.Builder
				b.AddUint16(m.greaseExtension)
				b.AddUint16(0x0000)
				extensionBytes = append(extensionBytes, b)
			}
			if m.earlyData {
			...
			if m.reversedExtensionOrder {
				extensionLen := len(extensionBytes) - 1
				if containsPSK {
					extensionLen -= 1
				}
				for i := 0; i < extensionLen; i += 1 {
					if i == extensionLen-i {
						break
					}
					extensionBytes[i], extensionBytes[extensionLen-i] = extensionBytes[extensionLen-i], extensionBytes[i]
				}
			}
			for _, builder := range extensionBytes {
				b.AddBytes(builder.BytesOrPanic())
			}
...
func (m *serverHelloMsg) unmarshal(data []byte, conn *Conn) bool {
	*m = serverHelloMsg{raw: data}
	...
	for !extensions.Empty() {
		...
		if isHRR {
			conn.helloRetryRequestExtensions = append(conn.helloRetryRequestExtensions, Extension{Extension: extension, Data: extData})
		} else {
			conn.serverExtensions = append(conn.serverExtensions, Extension{Extension: extension, Data: extData})
		}
...
func (m *encryptedExtensionsMsg) unmarshal(data []byte, conn *Conn) bool {
	*m = encryptedExtensionsMsg{raw: data}
	...
	for !extensions.Empty() {
		...
		// Here we have the Extension and Data before unknown ones are discarded
		conn.serverEncryptedExtensions = append(conn.serverEncryptedExtensions, Extension{Extension: extension, Data: extData})
...
func (m *certificateRequestMsgTLS13) unmarshal(data []byte, conn *Conn) bool {
	*m = certificateRequestMsgTLS13{raw: data}
	...
	for !extensions.Empty() {
		...
		conn.serverCertRequestExtensions = append(conn.serverCertRequestExtensions, Extension{Extension: extension, Data: extData})
